...
public class C2Product extends Configured implements Tool {
	...
	public int run(String arg[]) throws Exception
	{
		...
		Job job = Job.getInstance(conf, "C2 loss: product by age group");
		
		job.setJarByClass(C2Product.class);
		
		job.setMapperClass(C2ProductM.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(Text.class);
		
		job.setReducerClass(C2ProductR.class);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);
		
		job.setPartitionerClass(C2ProductP.class);
	...
	{
		ToolRunner.run(new Configuration(), new C2Product(), ar);
		System.exit(0);
	}
